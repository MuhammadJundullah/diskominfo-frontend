...
const Kontak = () => {
  ...
  const imgAnimation = useSpring({
    ...
    transform: imgInView ? "translateX(0)" : "translateX(20px)",
    config: { tension: 280, friction: 60 },
  });

  // Animasi untuk div konten
  const divAnimation = useSpring({
    opacity: divInView ? 1 : 0,
    transform: divInView ? "translateX(0)" : "translateX(-20px)",
    config: { tension: 280, friction: 60 },
  });

  return (
    <div className="bg-white lg:grid lg:grid-cols-5 sm:py-32 sm:px-10">
      {/* Gambar dengan animasi */}
      <animated.div
        ref={imgRef} // Menambahkan ref untuk gambar
        className="relative block h-32 lg:col-span-2 lg:h-full"
        style={imgAnimation} // Aplikasikan animasi gambar
      >
        <img
